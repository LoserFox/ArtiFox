...
var image = function get_image(data){
    ...
    if (request_user.indexOf(data.user_id)==-1){
        request_user.push(data.user_id)
    }else{
        global.http.Send_Packet("/send_group_msg", {
            "group_id": data.group_id,
            "message" : "呜呜呜...不要再催了，很快就好了"
        });
        return
    }
    console.log(request_user)
    ...
    global.http.Get_Packet("http://www.dmoe.cc/random.php",{},(err,rea,body)=>{
        ...
        .fontSize(20) 
        .font(path.join("font","SourceHanSansCN-Regular.otf"))
        .drawText(20, 345, do_something[today[3]].name+"  "+do_something[today[3]].good )
        .drawText(20, 370, do_something[today[4]].name+"  "+do_something[today[4]].good )
        .drawText(20, 445, do_something[today[5]].name+"  "+do_something[today[5]].bad )
        .drawText(20, 470, do_something[today[6]].name+"  "+do_something[today[6]].bad )
            ...
            .write(img_path, function (err) {
                if(err){console.error(err);return;}
                    ...
                    .write(img_path, function (err) {
                       ...
                       request_user[request_user.indexOf(data.user_id)]=undefined
                })
            })
        })
...
function today_random(data){
    if (global.Save_JSON.today == undefined) {
      ...
      if (user_data[data.user_id] == undefined){
        user_data[data.user_id] = [undefined,0,0,1,2,3,4]
      }
      if (user_data[data.user_id][0] == undefined) {
        var random_num = [random(100),random(100),random(100),random(102),random(102),random(102),random(102)]
        random_num[2]=user_data[data.user_id][1]+random_num[1]
        console.log(random_num)
        user_data[data.user_id] = random_num
        global.JSON.JSON_save(global.Save_JSON)
      }else{
        var random_num = user_data[data.user_id]
      }
      return random_num
}
function greetings() {
